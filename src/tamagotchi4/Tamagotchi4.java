...
public class Tamagotchi4 implements Runnable {
    ...
    public static BufferedImage character, characterB, characterS, characterM;
    public BufferedImage[] player = new BufferedImage[4];
    public boolean blink = false, play = false, wash = false, eat = false, sleep = false;
    private int x, y;
    private final int scale = 1;
    public static String name = "";

    public static int clean = 950, hunger = 950, sleepy = 950, happiness = 950;
    public static final int maxClean = 950, maxHunger = 950, maxSleepy = 950, maxHappiness = 950;
    ...
    public static BufferedImage food, bed, shower, smiley, ball, fly;
    ...
    public void save(String name, int hunger, int sleepy, int happy, int clean) throws FileNotFoundException, UnsupportedEncodingException {
    ...
    private void render() throws FileNotFoundException, UnsupportedEncodingException {
        save(name, hunger, sleepy, happiness, clean);
        ...
        if (loading == true) {
            ...
            if (menu == false && pause == false) {
                ...
                if (blink == true || sleep == true) {
                    ...
                    g.drawImage(player[1], x , y , character.getWidth() * scale, character.getHeight() * scale, null);
                } else if (wash == true) {
                    g.drawString(name, cham.x +cham.width / 2, cham.y - 10);
                    g.drawImage(player[2], x, y, character.getWidth() * scale, character.getHeight()* scale, null);
                } else if (eat == true) {
                    g.drawString(name, cham.x +cham.width / 2, cham.y - 10);
                    g.drawImage(player[3],  x, y, character.getWidth() * scale, character.getHeight()* scale, null);
                } else if (play == true) {
                    g.drawString(name, cham.x +cham.width / 2, cham.y - 30);
                    g.drawImage(player[0],  x, y - 20, character.getWidth() * scale, character.getHeight()* scale, null);
                } else {
                    g.drawString(name, cham.x +cham.width / 2, cham.y - 10);
                    g.drawImage(player[0],  x, y, character.getWidth() * scale, character.getHeight()* scale, null);
                }
    ...
    public void run() {
        ...
        int fps = 60;
        int b = 0, p = 0, s = 0, w = 0, e = 0, counter = 0;
        while (run) {
            ...
            if ((double) (time - lastTime) / 1000 > (double) 1 / fps) {
                ...
                try {
                    ...
                    Logger.getLogger(Tamagotchi4.class.getName()).log(Level.SEVERE, null, ex);
